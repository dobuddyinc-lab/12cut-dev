...
#### F. Vercel 배포 (레거시 기록 — **현재 미사용**)

> §1 프로젝트 개요 기준 **Production은 Cloudflare Pages**만 유지한다.

| 항목 | 값 |
|---|---|
| Vercel 계정 | jay11111762 |
| Project | 12cut |
| 당시 Production URL | https://12cut.vercel.app |
| 당시 배포 방식 | `npx vercel --yes --prod` (수동) |
...
├── domain-report.html      # 내부용 도메인 전략 리포트 (/domain-report.html)
├── _headers                # Cloudflare Pages 응답 헤더
├── wrangler.toml           # 로컬 `wrangler pages dev` 출력 루트 (`.` )
├── AGENTS.md               # AI/에이전트용 베스트 프랙티스
├── README.md               # 저장소 진입점 (실행·배포 요약)
├── .vercel/                # 과거 시도 시에만 존재 — .gitignore
├── MD/
│   └── CONTEXT.md          # 본 문서 (통합 레퍼런스)
├── scripts/
